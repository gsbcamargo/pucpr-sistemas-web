...
public class SolicitacaoRest {
	...
	@Path("{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response obterSolicitacaoPorId(@PathParam("id") Integer id) throws Exception {
		try {
	...
	public Response incluirSolicitacao(Solicitacao solicitacao) {
		try {
			Solicitacao solicit = Banco.incluirSolicitacao(solicitacao);
			return Response.ok().entity(solicit).build();
		} catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity("{ \"mensagem\" : \"Falha na inclusao da solicitacao!\" , \"detalhe\" :  \"" + e.getMessage()
							+ "\"  }")
					.build();
		}
	}
	...
	public Response alterarSolicitacao(@PathParam("id") Integer id, Solicitacao solicitacao) {
		try {
			...
			Solicitacao solicit = Banco.alterarSolicitacao(id, solicitacao);
			return Response.ok().entity(solicit).build();
		} catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity("{ \"mensagem\" : \"Falha na alteracao da Solicitacao!\" , \"detalhe\" :  \"" + e.getMessage()
							+ "\"  }")
					.build();
		}
	}
